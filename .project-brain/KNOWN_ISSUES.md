...
| `[SMS-CHANNEL-DISPATCH-DISCONNECT-AND-VOID]` | `NotificationDispatcher.php:141-148`, `NotificationService.php:257` | NotificationDispatcher routing haritasında 'sms' kanalı tanımlı değil (Unsupported channel hatası); NotificationService ise sahte başarı dönüp SMS atmıyor | SMS Bildirimlerinin Asla Gönderilmemesi (SMS Blackhole) |
| `[PROD-QUEUE-WORKER-NOTIFICATIONS-BLACKOUT]` | `SendNotificationJob.php:46`, `docker-compose.production.yml:146` | SendNotificationJob 'notifications' kuyruğuna atılıyor; canlı Docker worker ise sadece '--queue=default' dinliyor | Tüm Asenkron E-posta/WhatsApp/Telegram İşlerinin Kuyrukta Takılı Kalması |
