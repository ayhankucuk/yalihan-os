...
class GuardRoutesV2Command extends Command
{
    protected $signature = 'guard:routes:v2
        {--report=docs/reports/hygiene_route_integrity_v11_0 : Report target (base path or .md/.json)}';

    protected $description = 'Route integrity guard (v2) with report export';

    public function handle(): int
    {
        File::ensureDirectoryExists(storage_path('logs'));

        $this->line('🔍 guard:routes:v2 starting...');
        ...
        $sourceMd = base_path('docs/reports/hygiene_route_integrity_v11_0.md');
        if (! File::exists($sourceMd)) {
            $this->error('Route integrity report not generated');
        ...
        if (count($issues) > 0) {
            $this->error('guard:routes:v2 FAILED');

            return 1;
        }

        $this->info('guard:routes:v2 PASSED');
    ...
    private function resolveTargets(string $reportOption): array
    {
        ...
        if ($path === '') {
            $path = 'docs/reports/hygiene_route_integrity_v11_0';
        }
