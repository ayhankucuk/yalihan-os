...
<div class="relative group">
    ...
    <div class="relative">
        ...
        <textarea 
            ...
            {{ $attributes->merge([
                'class' => 'peer w-full px-4 py-3.5 text-base
                           ' . ($icon ? 'pl-12' : 'pl-4') . '
                           border-2 ' . ($hasError ? 'border-red-500' : 'border-gray-300 dark:border-gray-600') . '
                           rounded-xl
        ...
        @if($label && $floating)
        <label for="{{ $inputId }}"
               class="absolute {{ $icon ? 'left-12' : 'left-4' }} top-3.5
                      ...
                      {{ old($name, $value) ? 'top-0 -translate-y-1/2 text-xs bg-gray-50 dark:bg-gray-800 px-2 py-0.5 rounded font-semibold' : '' }}">
            @if($icon)
