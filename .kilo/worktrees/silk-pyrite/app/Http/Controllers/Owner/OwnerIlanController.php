...
class OwnerIlanController extends Controller
{
    ...
    /**
     * Portföy düzenleme formunu gösterir.
     *
     * SAB v6.1.2 — Sprint 3.4.3
     * Ownership kontrolü: owner sadece kendi ilanını düzenleyebilir.
     */
    public function edit(Ilan $ilan): View
    {
        $user = auth()->user();

        // Tenant isolation: sadece kendi ilanına erişim
        $ilan = Ilan::with(['il', 'ilce', 'mahalle', 'anaKategori', 'altKategori'])
            ->where('id', $ilan->id)
            ->where('user_id', $user->id)
            ->firstOrFail();
    ...
    public function update(UpdateOwnerIlanRequest $request, Ilan $ilan): RedirectResponse
    {
        ...
        // Tenant isolation: sadece kendi ilanını güncelleyebilir
        $ilan = Ilan::where('id', $ilan->id)
            ->where('user_id', $user->id)
            ->firstOrFail();
        ...
        $ilanService = app(IlanService::class);
        $result = $ilanService->updateListing($ilan, $data);

        return to_route('owner.ilanlar.show', $ilan->id)
            ->with('success', $result['message'] ?? 'Değişiklikler kaydedildi.');
    }

    /**
     * Portföyü soft-delete ile siler.
     *
     * SAB v6.1.2 — Sprint 3.4.3
     * Write authority: IlanService::deleteListing() → IlanCrudService::destroy()
     * IlanCrudService::destroy() zaten soft-delete kullanır.
     */
    public function destroy(Ilan $ilan): RedirectResponse
    {
        $user = auth()->user();

        // Tenant isolation: sadece kendi ilanını silebilir
        $ilan = Ilan::where('id', $ilan->id)
            ->where('user_id', $user->id)
            ->firstOrFail();
        ...
        return to_route('owner.ilanlar.index')
            ->with('success', 'İlan başarıyla kaldırıldı.');
    }
}
