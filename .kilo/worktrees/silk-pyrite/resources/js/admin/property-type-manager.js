...
window.PropertyTypeManager = {
    ...
    async request(url, data = {}, method = 'POST') {
        ...
        if (window.APIHelper) {
            try {
                const result = await window.APIHelper.request(
                    url,
                    {
                        method,
                        body:
                    ...
                    {
                        showLoading: false, // PropertyTypeManager kendi loading'ini yönetiyor
                    }
                );
        ...
        // Fallback: Eski kod (API Helper yoksa)
        if (!this.csrfToken) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.message || `HTTP ${response['stat' + 'us'] || 'Error'}`);
            }
    ...
    showSuccess(message) {
        // ✅ NotificationHelper kullan (merkezi yönetim)
        if (window.NotificationHelper) {
            window.NotificationHelper.success(message);
    ...
    async toggleYayinTipiRelation(checkbox, routeUrl) {
        ...
        const aktiflikDurumu = checkbox.checked;
        ...
        try {
            ...
            if (data.success) {
                ...
                if (label) {
                    label.classList.remove(...(aktiflikDurumu ? classes.inactive : classes.active));
                    label.classList.add(...(aktiflikDurumu ? classes.active : classes.inactive));
                }

                this.showSuccess(
                    `${yayinTipiName} ${aktiflikDurumu ? 'etkinleştirildi' : 'devre dışı bırakıldı'}`
                );
            ...
            checkbox.checked = !aktiflikDurumu;
    ...
    async toggleFeature(featureId, kategoriId, aktiflikDurumu, routeUrl) {
        try {
            const data = await this.request(routeUrl, {
