    ...
    <div style="background:#0F2A5C; padding-top:7rem; padding-bottom:2.5rem;">
        <div class="max-w-7xl mx-auto px-6 md:px-12">
            <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.75rem;font-size:0.75rem;font-weight:500;color:rgba(255,255,255,0.55);">
                ...
                <span>›</span>
                <span style="color:#fff;">Uluslararası</span>
            </div>
            ...
            <p style="font-size:0.95rem;color:rgba(255,255,255,0.65);max-width:40rem;">
                Yalıhan güvencesiyle dünya başkentlerinde yatırım fırsatları, oturum izinleri ve yüksek kira getirili portföyler.
            </p>
        </div>
    </div>
                    ...
                    <div class="mb-8">
                        ...
                        <div class="space-y-3">
                            ...
                            @if(empty($filters['countries']))
                                @foreach(['uk' => 'Birleşik Krallık', 'greece' => 'Yunanistan', 'portugal' => 'Portekiz', 'uae' => 'Birleşik Arap Emirlikleri'] as $val => $label)
                                    <label class="flex items-center gap-3 cursor-pointer group">
                                        <input type="checkbox" name="country[]" value="{{ $val }}"
                                               {{ in_array($val, (array)($selectedFilters['country'] ?? [])) ? 'checked' : '' }}
                                               class="custom-checkbox">
                                        <span class="text-body-sm text-on-surface group-hover:text-primary transition-colors">{{ $label }}</span>
                                    </label>
                                @endforeach
                            @endif
                        </div>
                    ...
                    @forelse($featured as $index => $ilan)
                        ...
                        <article class="bg-white rounded-2xl overflow-hidden border border-outline-variant/30 hover:shadow-xl transition-all duration-300 group flex flex-col">
                            <div class="relative h-56 overflow-hidden bg-slate-100">
                                ...
                                @else
                                    <div class="w-full h-full" style="background:linear-gradient(135deg,#0F2A5C 0%,#1a3a6b 100%);"></div>
                                @endif
