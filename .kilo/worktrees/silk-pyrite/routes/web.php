...
Route::prefix('preferences')->name('preferences.')->group(function () {
    Route::post('/locale', [\App\Http\Controllers\Frontend\PreferenceController::class, 'setLocale'])->name('locale');
    Route::post('/currency', [\App\Http\Controllers\Frontend\PreferenceController::class, 'setCurrency'])->name('currency');
});

// 🌐 International Landing Pages (Locale Prefixed)
Route::prefix('{locale}')->where(['locale' => '[a-z]{2}'])->group(function () {
    Route::get('/invest-in-turkey', [\App\Http\Controllers\Public\InternationalLandingController::class, 'investInTurkey'])->name('public.invest-in-turkey');
    Route::get('/golden-visa-greece', [\App\Http\Controllers\Public\InternationalLandingController::class, 'goldenVisaGreece'])->name('public.golden-visa-greece');
    Route::get('/uk-investment', [\App\Http\Controllers\Public\InternationalLandingController::class, 'ukPropertyInvestment'])->name('public.uk-investment');
    Route::get('/calculator', [\App\Http\Controllers\Public\InternationalLandingController::class, 'rentalIncomeCalculator'])->name('public.calculator');
});


// ✅ Public Calendar Feed (Phase Q)
Route::get('/calendar/ilan/{token}.ics', [\App\Http\Controllers\Public\IlanCalendarFeedController::class, 'show'])
    ->middleware(['throttle:60,1'])
    ->name('calendar.feed');

// 🔒 [YALIHAN_REPORTING_0206] Rapor Download - Signed URL (Public with Signature)
Route::get('/rapor/{ilan}/download', [\App\Http\Controllers\Admin\IlanRaporController::class, 'show'])
...
Route::prefix('yazliklar')->name('villas.')->group(function () {
    ...
    // Villa detail page
    Route::get('/deal-radar/{ilanId}', [\App\Http\Controllers\Advisor\DealRadarController::class, 'show'])->name('advisor.deal-radar.show');

    // AI Owner Discovery Engine
    Route::get('/owner-opportunities', [\App\Http\Controllers\Advisor\OwnerDiscoveryController::class, 'index'])->name('advisor.owner-opportunities');
    Route::post('/owner-opportunities/run', [\App\Http\Controllers\Advisor\OwnerDiscoveryController::class, 'runDiscovery'])->name('advisor.owner-opportunities.run');

    // AI Market Valuation Engine
    Route::get('/market-valuation', [\App\Http\Controllers\Advisor\MarketValuationController::class, 'index'])->name('advisor.market-valuation');
    Route::post('/market-valuation/fetch', [\App\Http\Controllers\Advisor\MarketValuationController::class, 'fetch'])->name('advisor.market-valuation.fetch');
    // AI Market Intelligence Engine
    Route::get('/market-intelligence', [\App\Http\Controllers\Advisor\MarketIntelligenceAdvisorController::class, 'index'])->name('advisor.market-intelligence');
...
Route::get('/simple-create', function () {
    ...
    $kisiler = \App\Models\Kisi::where('aktiflik_durumu', 1)->get(); // ✅ SAB: durum → aktiflik_durumu
    $danismanlar = \App\Models\User::where('role_id', 2)->where('aktiflik_durumu', 1)->get();
...
*/
    Route::get('/{id}/investment-report', function ($id) {
        $ilan = \App\Models\Ilan::findOrFail($id);
        return view('ilanlar.exports.investment-report', compact('ilan'));
    })->name('investment.report');
});

// Temiz kategori URL'leri — /arsa, /konut
Route::get('/arsa', function (\Illuminate\Http\Request $request, \App\Services\CurrencyConversionService $currency) {
    $request->merge(['kategori_slug' => 'arsa-arazi']);
    return app(\App\Http\Controllers\IlanPublicController::class)->index($request, $currency);
})->name('arsa');

Route::get('/konut', function (\Illuminate\Http\Request $request, \App\Services\CurrencyConversionService $currency) {
    $request->merge(['kategori_slug' => 'konut']);
    return app(\App\Http\Controllers\IlanPublicController::class)->index($request, $currency);
})->name('konut');

Route::get('/satilik', function (\Illuminate\Http\Request $request, \App\Services\CurrencyConversionService $currency) {
    $request->merge(['islem_tipi' => 'satis', 'kategori_slug' => 'konut']);
    return app(\App\Http\Controllers\IlanPublicController::class)->index($request, $currency);
})->name('satilik');

Route::get('/kiralik', function (\Illuminate\Http\Request $request, \App\Services\CurrencyConversionService $currency) {
    $request->merge(['islem_tipi' => 'kiralama', 'kategori_slug' => 'konut']);
    return app(\App\Http\Controllers\IlanPublicController::class)->index($request, $currency);
})->name('kiralik');

// AI Advisor (Public)
Route::get('/ai-advisor', [\App\Http\Controllers\Public\ConversationalAdvisorPublicController::class, 'index'])->name('public.conversational');
Route::post('/ai-advisor/query', [\App\Http\Controllers\Public\ConversationalAdvisorPublicController::class, 'query'])
    ->middleware('throttle:60,1')
    ->name('public.conversational.query');
Route::post('/ai-advisor/clear', [\App\Http\Controllers\Public\ConversationalAdvisorPublicController::class, 'clearHistory'])
    ->name('public.conversational.clear');

// Frontend Portfolio Routes
Route::prefix('portfolio')->name('frontend.portfolio.')->group(function () {
    Route::get('/', function () {
        $properties = \App\Models\Ilan::byYayinDurumu('Aktif')
            ->with(['il', 'ilce', 'etiketler'])
            ->paginate(12);

        $stats = [
            'total_properties' => \App\Models\Ilan::count(),
            'active_properties' => \App\Models\Ilan::byYayinDurumu('Aktif')->count(),
            'total_value' => \App\Models\Ilan::byYayinDurumu('Aktif')->sum('fiyat') / 1000000,
...
Route::middleware('auth')->group(function () {
    ...
    */
    ...
    Route::middleware(['web', 'auth', 'throttle:60,1'])->group(function () {
        ...
        Route::get('/admin/monitor/ai-stats', [\App\Http\Controllers\Admin\AiMonitorController::class, 'index'])->name('admin.monitor.ai-stats');
    ...
    Route::middleware(['web', 'auth', 'throttle:60,1'])->group(function () {
        ...
        Route::get('/command-center', [App\Http\Controllers\Advisor\AdvisorCommandCenterController::class, 'index'])->name('advisor.command-center');
        Route::get('/command-center/fetch', [App\Http\Controllers\Advisor\AdvisorCommandCenterController::class, 'fetch'])->name('advisor.command-center.fetch');

        Route::get('/advisor/opportunities', [\App\Http\Controllers\Advisor\OpportunityInboxController::class, 'index'])->name('advisor.opportunities');
        Route::get('/opportunity-inbox/fetch', [App\Http\Controllers\Advisor\OpportunityInboxController::class, 'fetch'])->name('opportunity-inbox.fetch');
    ...
    // AI Deal Radar
    Route::get('/advisor/listings/{listing}/buyer-matches', [\App\Http\Controllers\Advisor\BuyerMatchQueueController::class, 'index'])->name('advisor.buyer-match-queue');
        Route::get('/advisor/listings/{listing}/buyer-matches/fetch', [\App\Http\Controllers\Advisor\BuyerMatchQueueController::class, 'fetch'])->name('advisor.buyer-match-queue.fetch');

        Route::get('/advisor/listing/{ilan}/buyers', [\App\Http\Controllers\Advisor\BuyerMatchController::class, 'index'])->name('advisor.listing.buyers');

        // AI Conversational Advisor
        Route::get('/advisor/conversational', [App\Http\Controllers\Advisor\ConversationalAdvisorController::class, 'index'])->name('advisor.conversational');
        Route::post('/advisor/conversational/query', [App\Http\Controllers\Advisor\ConversationalAdvisorController::class, 'query'])
            ->middleware('throttle:60,1')
            ->name('advisor.conversational.query');

        // AI Advisor Analytics
        Route::get('/advisor/analytics', [App\Http\Controllers\Advisor\AdvisorAnalyticsController::class, 'index'])->name('advisor.analytics');


        // AI Deal Radar Engine
        Route::get('/advisor/deal-radar', [\App\Http\Controllers\Advisor\DealRadarController::class, 'index'])->name('advisor.deal-radar');
        Route::get('/advisor/deal-radar/fetch', [\App\Http\Controllers\Advisor\DealRadarController::class, 'fetch'])->name('advisor.deal-radar.fetch');

        // AI Seller Strategy Engine
        Route::get('/advisor/listings/{listing}/seller-strategy', [\App\Http\Controllers\Advisor\SellerStrategyController::class, 'view'])->name('advisor.seller-strategy');
        Route::get('/advisor/listings/{listing}/seller-strategy/fetch', [\App\Http\Controllers\Advisor\SellerStrategyController::class, 'fetch'])->name('advisor.seller-strategy.fetch');

        // AI Broker Copilot
        Route::get('/advisor/copilot', [\App\Http\Controllers\Advisor\CopilotController::class, 'index'])->name('advisor.copilot');

        // AI Price Advisor
        Route::get('/advisor/listing/{ilan}/price', [\App\Http\Controllers\Advisor\PriceAdvisorController::class, 'index'])->name('advisor.listing.price');

        // AI Portfolio Doctor (Phase 21 UI)
        Route::get('/advisor/portfolio/doctor', [\App\Http\Controllers\Advisor\PortfolioDoctorController::class, 'index'])->name('advisor.portfolio-doctor');
        Route::get('/advisor/listing/{ilan}/diagnostics', [\App\Http\Controllers\Advisor\PortfolioDoctorController::class, 'diagnostics'])->name('advisor.listing.diagnostics');
    });
...
Route::prefix('owner')->name('owner.')->middleware(['web', 'throttle:20,1'])->group(function () {
    ...
    Route::get('/login',          [\App\Http\Controllers\Owner\OwnerAuthController::class, 'showLoginForm'])->name('login');
    Route::post('/auth/send',     [\App\Http\Controllers\Owner\OwnerAuthController::class, 'sendLoginLink'])->name('auth.send');
    Route::get('/auth/verify',    [\App\Http\Controllers\Owner\OwnerAuthController::class, 'verifyToken'])->name('auth.verify');
...
Route::prefix('owner')->name('owner.')->middleware(['web', 'check.owner'])->group(function () {
    ...
    // Çıkış
    Route::post('/logout', [\App\Http\Controllers\Owner\OwnerAuthController::class, 'logout'])->name('logout');
    ...
    Route::prefix('reports')->name('reports.')->group(function () {
        Route::get('/',                   [\App\Http\Controllers\Owner\OwnerReportController::class, 'index'])->name('index');
        Route::post('/export',            [\App\Http\Controllers\Owner\OwnerReportController::class, 'export'])->name('export');
        Route::get('/{export}/download',  [\App\Http\Controllers\Owner\OwnerReportController::class, 'download'])->name('download');
    });

    // 🏠 İlanlarım (Task #15)
    Route::get('/ilanlar', [\App\Http\Controllers\Owner\OwnerIlanController::class, 'index'])->name('ilanlar.index');
    Route::get('/ilanlar/create', [\App\Http\Controllers\Owner\OwnerIlanController::class, 'create'])->name('ilanlar.create');
    Route::post('/ilanlar', [\App\Http\Controllers\Owner\OwnerIlanController::class, 'store'])->name('ilanlar.store');
    Route::get('/ilanlar/{ilan}/edit', [\App\Http\Controllers\Owner\OwnerIlanController::class, 'edit'])->name('ilanlar.edit');
    Route::put('/ilanlar/{ilan}', [\App\Http\Controllers\Owner\OwnerIlanController::class, 'update'])->name('ilanlar.update');
    Route::delete('/ilanlar/{ilan}', [\App\Http\Controllers\Owner\OwnerIlanController::class, 'destroy'])->name('ilanlar.destroy');
    Route::get('/ilanlar/{id}', [\App\Http\Controllers\Owner\OwnerIlanController::class, 'show'])->name('ilanlar.show');

    // 📷 Portföy Fotoğrafları (Sprint 3.4.2)
    Route::post('/ilanlar/{ilan}/photos', [\App\Http\Controllers\Owner\OwnerPhotoController::class, 'upload'])->name('ilanlar.photos.upload');
    Route::delete('/ilanlar/{ilan}/photos/{photo}', [\App\Http\Controllers\Owner\OwnerPhotoController::class, 'delete'])->name('ilanlar.photos.delete');

    // 🧠 Portföy Hazırlık Analizi (Sprint 3.4.3)
    Route::get('/ilanlar/{ilan}/readiness', [\App\Http\Controllers\Owner\OwnerIntelligenceController::class, 'readiness'])->name('ilanlar.readiness');

    // 📩 Teklifler & Talepler (Task #16)
    Route::get('/teklifler', [\App\Http\Controllers\Owner\OwnerTeklifController::class, 'index'])->name('teklifler.index');
    Route::get('/teklifler/{id}', [\App\Http\Controllers\Owner\OwnerTeklifController::class, 'show'])->name('teklifler.show');

    // 💬 Danışmanla İletişim (Task #17)
    Route::get('/mesajlar', [\App\Http\Controllers\Owner\OwnerMesajController::class, 'index'])->name('mesajlar.index');
    Route::post('/mesajlar', [\App\Http\Controllers\Owner\OwnerMesajController::class, 'store'])->name('mesajlar.store');

    // 📁 Belgelerim (Task #18)
    Route::get('/belgeler', [\App\Http\Controllers\Owner\OwnerBelgeController::class, 'index'])->name('belgeler.index');
    Route::get('/belgeler/{id}/download', [\App\Http\Controllers\Owner\OwnerBelgeController::class, 'download'])->name('belgeler.download');
