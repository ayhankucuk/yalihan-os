...
use App\Http\Controllers\Admin\DescriptionDraftController;
...
Route::middleware(['auth', 'admin', 'role:admin'])->prefix('admin')->group(function () {
    ...
    // AI Description Draft Routes
    // Pipeline: Context Builder → LLM → Draft → Owner Review → Accept → Persist
    Route::prefix('ilan-ai/draft')->name('admin.ilan-ai.draft.')->group(function () {

        // Generate AI Description Draft
        Route::post('/generate/{ilan}', [DescriptionDraftController::class, 'generate'])->name('generate');

        // Get latest draft for review
        Route::get('/{ilan}', [DescriptionDraftController::class, 'show'])->name('show');

        // Get draft history
        Route::get('/{ilan}/history', [DescriptionDraftController::class, 'history'])->name('history');

        // Approve draft and persist to ilan.aciklama
        Route::post('/{draft}/approve', [DescriptionDraftController::class, 'approve'])->name('approve');

        // Reject draft
        Route::post('/{draft}/reject', [DescriptionDraftController::class, 'reject'])->name('reject');
    });
