...
@section('content')
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="rounded-xl bg-white p-6 shadow-sm dark:bg-slate-900 dark:shadow-none">
            <form action="{{ route('admin.takim.projeler.update', $proje) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="proje_adi" id="proje_adi"
                            value="{{ old('proje_adi', $proje->proje_adi) }}" required
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="aciklama" id="aciklama" rows="3"
                            class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 transition-all duration-200 focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-slate-900 dark:text-white">{{ old('aciklama', $proje->aciklama) }}</textarea>
                        @error('aciklama')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="baslangic_tarihi" id="baslangic_tarihi"
                            value="{{ old('baslangic_tarihi', $proje->baslangic_tarihi ? \Carbon\Carbon::parse($proje->baslangic_tarihi)->format('Y-m-d') : '') }}"
                            required
                            class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 transition-all duration-200 focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-slate-900 dark:text-white">
                    ...
                    <div>
                        ...
                        <input type="date" name="bitis_tarihi" id="bitis_tarihi"
                            value="{{ old('bitis_tarihi', $proje->bitis_tarihi ? \Carbon\Carbon::parse($proje->bitis_tarihi)->format('Y-m-d') : '') }}"
                            class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 transition-all duration-200 focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-slate-900 dark:text-white">
                        @error('bitis_tarihi')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="yayin_durumu" id="yayin_durumu" required
                            class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 transition-all duration-200 focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-slate-900 dark:text-white">
                            @php $yd = old('yayin_durumu', $proje->yayin_durumu); @endphp
                            <option value="planlama" @selected($yd == 'planlama')>
                                Planlama</option>
                            <option value="devam_ediyor" @selected($yd == 'devam_ediyor')>Devam Ediyor
                            </option>
                            <option value="beklemede" @selected($yd == 'beklemede')>
                                Beklemede</option>
                            <option value="tamamlandi" @selected($yd == 'tamamlandi')>Tamamlandı</option>
                            <option value="iptal" @selected($yd == 'iptal')>İptal</option>
                        </select>
                        @error('yayin_durumu')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="oncelik" id="oncelik" required
                            class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 transition-all duration-200 focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-slate-900 dark:text-white">
                            <option value="dusuk" {{ old('oncelik', $proje->oncelik) == 'dusuk' ? 'selected' : '' }}>Düşük
                            </option>
                            <option value="orta" {{ old('oncelik', $proje->oncelik) == 'orta' ? 'selected' : '' }}>Orta
                            </option>
                            <option value="yuksek" {{ old('oncelik', $proje->oncelik) == 'yuksek' ? 'selected' : '' }}>
                                Yüksek</option>
                            <option value="kritik" {{ old('oncelik', $proje->oncelik) == 'kritik' ? 'selected' : '' }}>
                                Kritik</option>
                        </select>
                        @error('oncelik')
                            <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                        ...
                        <select name="user_id" id="user_id" required
                            ...
                            @foreach ($users as $user)
                                <option value="{{ $user->id }}"
                                    {{ old('user_id', $proje->user_id) == $user->id ? 'selected' : '' }}>
                                    {{ $user->name }}</option>
                            @endforeach
                        </select>
                        @error('user_id')
                    ...
                    <div>
                        ...
                        <input type="number" name="budget" id="budget" value="{{ old('budget', $proje->budget) }}"
                            min="0" step="0.01"
                            class="w-full rounded-lg border border-gray-300 bg-white px-4 py-2 text-gray-900 transition-all duration-200 focus:border-transparent focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-slate-900 dark:text-white"
                            placeholder="0.00">
                        @error('budget')
