...
class TemplateResolverTest extends TestCase
{
    ...
    {
        // Arrange: önce kategorileri oluştur
        $kategori = IlanKategori::create(['name' => 'Test Koleksiyon Kat', 'seviye' => 0, 'aktiflik_durumu' => true]);
        $otherKategori = IlanKategori::create(['name' => 'Diger Kat', 'seviye' => 0, 'aktiflik_durumu' => true]);
    ...
    {
        // Arrange: önce kategori oluştur
        $kategori = IlanKategori::create(['name' => 'Siralama Kat', 'seviye' => 0, 'aktiflik_durumu' => true]);
        $kategoriId = $kategori->id;
    ...
    public function resolve_by_junction_is_deterministic_regardless_of_slug()
    {
        ...
        $k1 = IlanKategori::create(['name' => 'Kat K1', 'seviye' => 0, 'aktiflik_durumu' => true]);
        $k2 = IlanKategori::create(['name' => 'Kat K2', 'seviye' => 0, 'aktiflik_durumu' => true]);
    ...
    {
        $kategori = IlanKategori::create(['name' => 'Kat Gunluk', 'seviye' => 0, 'aktiflik_durumu' => true]);
        ...
        $resolved = $this->resolver->resolveByJunction($template->id, $kategori->id);
    ...
    {
        $kategori = IlanKategori::create(['name' => 'Kat Kucuk Villa', 'seviye' => 0, 'aktiflik_durumu' => true]);
        ...
        // Act + Assert: request.kategori_id farklı => TemplateCategoryMismatchException
        $this->expectException(TemplateCategoryMismatchException::class);

        $this->resolver->resolveByJunction($template->id, $kategori->id + 9999);
    }

    /** @test */
    public function resolve_by_junction_mismatch_exception_carries_context()
    {
        $kategori = IlanKategori::create(['name' => 'Kat Villa', 'seviye' => 0, 'aktiflik_durumu' => true]);
        ...
        try {
            $this->resolver->resolveByJunction($template->id, 99999);
            ...
            $this->assertEquals(99999, $ctx['request_kategori_id']);
            $this->assertEquals($kategori->id, $ctx['junction_kategori_id']);
        }
    }
