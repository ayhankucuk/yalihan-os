...
@php
    ...
    $ilAdi      = data_get($property, 'il.il_adi') ?? data_get($property, 'country_name', '');
    $ilceAdi    = data_get($property, 'ilce.ilce_adi') ?? data_get($property, 'city_name', '');
    $mahalleAdi = data_get($property, 'mahalle.mahalle_adi');
    if ($mahalleAdi && strtolower($mahalleAdi) === 'belirtilmemiş') { $mahalleAdi = null; }
    // Gösterim: "Mahalle, İlçe" (ör: Gündoğan, Bodrum) — il genellikle bilinir
    $fullLocation = collect([$mahalleAdi, $ilceAdi])->filter()->implode(', ') ?: ($ilAdi ?: 'Konum');
...
<article class="group bg-white dark:bg-slate-900 rounded-3xl shadow-md border border-gray-200 dark:border-slate-800 overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl dark:shadow-none dark:border-slate-700">
    ...
    <div class="p-6 space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-3 text-sm text-gray-600 dark:text-slate-200">
            @if ($bedrooms)
                <span class="inline-flex items-center gap-1"><span class="material-symbols-outlined">bed</span>{{ $bedrooms }} Oda</span>
            @endif
            @if ($bathrooms)
                <span class="inline-flex items-center gap-1"><span class="material-symbols-outlined">bathtub</span>{{ $bathrooms }} Banyo</span>
            @endif
            @if ($squareMeters)
                <span class="inline-flex items-center gap-1"><span class="material-symbols-outlined">straighten</span>{{ $squareMeters }} m²</span>
            @endif
        </div>
