...
class IlanCrudService
{
    ...
    public function __construct(
        ...
        private ?HermesService $hermesService = null,
    ) {}
    ...
    public function store(array $data): Ilan
    {
        ...
        // Hermes: Dispatch PortfolioCreated event for agent notifications
        $this->dispatchHermesEvent($ilan);
    ...
    public function update(Ilan $ilan, array $data): Ilan
    {
        ...
        $ilan = DB::transaction(function () use ($ilan, $data) {
            ...
            if (isset($data['yayin_durumu']) || isset($data['status'])) { // context7-ignore: legacy API fallback
                $targetEnum = IlanDurumu::normalize($data['yayin_durumu'] ?? $data['status']); // context7-ignore
                if ($targetEnum) {
    ...
    /**
     * PRIVATE: Handle ekstra_ozellikler JSON kolonu (T-UPS-V2-FULL — 2026-06-24)
     *
     * Kategori/dikey bazlı dinamik form alanlarını ekstra_ozellikler kolonuna yazar.
     * metadata (sistem meta verisi) ile karıştırılmamalı — bu domain alanlarıdır.
     *
     * Mevcut değerle merge yapılır; $data['ekstra_ozellikler'] varsa öncelik alır.
     */
    private function handleEkstraOzellikler(Ilan $ilan, array $data): void
    {
        ...
        // Kategori slug'ına göre dinamik alanları topla
        $kategoriSlug = strtolower($ilan->anaKategori->slug ?? '');
        $dynamic = [];

        if (str_starts_with($kategoriSlug, 'arsa')) {
            // Arsa bazlı alanlar
            foreach (['imar_durumu', 'kaks', 'taks', 'gabari', 'ada_no', 'parsel_no', 'tapu_tipi', 'yola_cephe', 'altyapi_elektrik', 'altyapi_su', 'altyapi_dogalgaz'] as $field) {
                if (isset($data[$field])) {
                    $dynamic[$field] = $data[$field];
                }
            }
        } elseif ($kategoriSlug === 'yazlık' || $kategoriSlug === 'yazlik') {
            // Turizm/yazlık bazlı alanlar (ilanlar kolonunda karşılığı olmayan)
            foreach (['sezon_baslangic', 'sezon_bitis', 'iptal_politikasi', 'evcil_hayvan_uygun', 'sigara_icilmez'] as $field) {
                if (isset($data[$field])) {
                    $dynamic[$field] = $data[$field];
                }
            }
        } elseif ($kategoriSlug === 'ticari' || $kategoriSlug === 'isyeri') {
            // Ticari bazlı alanlar
            foreach (['isyeri_tipi', 'kira_bilgisi', 'ciro_bilgisi', 'ruhsat_durumu', 'personel_kapasitesi'] as $field) {
                if (isset($data[$field])) {
                    $dynamic[$field] = $data[$field];
                }
            }
     ...
     *
     * Split-Brain Fix (T-UPS-V2 Seçenek A — 2026-06-15):
     * ilanlar tablosu SSOT'tur. Turizm alanları $data yerine $ilan
     * accessor'larından okunur; ilan_turizm_details salt mirror görevi görür.
     * Double-write yolu: $data → ilanlar (accessor mutator) → turizmDetail.
     */
    ...
    /**
     * Dispatch Hermes event for portfolio creation
     *
     * Hermes: Event-driven foundation for Team Hermes.
     * No external API calls. No financial mutations.
     */
    private function dispatchHermesEvent(Ilan $ilan): void
    {
        if ($this->hermesService === null) {
            // Hermes not available (e.g., in testing without binding)
            return;
        }

        try {
            $event = new PortfolioCreated($ilan, [
                'source' => 'IlanCrudService',
                'created_at' => now()->toIso8601String(),
            ]);
            $this->hermesService->receive($event);
        } catch (\Throwable $e) {
            // Hermes failures should not break the main flow
            Log::warning('[Hermes] Failed to dispatch PortfolioCreated', [
                'ilan_id' => $ilan->id,
                'error' => $e->getMessage(),
            ]);
        }
