        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a href="{{ route('ilanlar.index', ['ilce' => $ilce->id]) }}"
               ...
               style="border:1px solid rgba(10,22,40,0.1);color:var(--navy);">
                <x-icon name="konum" class="w-4 h-4 flex-shrink-0" style="color:var(--gold);" />
                {{ $ilce->ilce_adi }}
...
<section id="ai-explorer" class="py-12" style="background:var(--cream);">
    <div class="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div id="ilanGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            ...
            @endphp
            <article class="ilan-kart bg-white rounded-2xl overflow-hidden flex flex-col {{ $highlightId > 0 && $ilan->id == $highlightId ? 'highlight-card' : '' }}"
                     data-ilce="{{ is_object($ilan->ilce) ? ($ilan->ilce->ilce_adi ?? '') : '' }}"
                     data-kategori="{{ $ilan->anaKategori?->slug ?? '' }}"
                     data-baslik="{{ strtolower($ilan->baslik ?? '') }}"
                     data-fiyat="{{ $fiyat }}"
                     id="{{ $ilan->id == $highlightId ? 'highlighted-card' : '' }}">
                ...
                <div class="p-4 flex flex-col flex-1">
                    ...
                    <p class="text-xs text-slate-400 flex items-center gap-1 mb-3">
                        <x-icon name="konum" class="w-3.5 h-3.5 flex-shrink-0" style="color:var(--gold);" />
                        {{ $lokasyon }}
                    </p>
                    @else
                    <p class="mb-3"></p>
                    ...
                    <div class="flex gap-3 text-xs text-slate-500 mb-4">
                        ...
                        <span class="flex items-center gap-1.5">
                            <x-icon name="alan" class="w-3.5 h-3.5 text-slate-400" />
                            {{ number_format($ilan->net_m2, 0, ',', '.') }} m²
                        </span>
                        @endif
                        @if($ilan->oda_sayisi)
                        <span class="flex items-center gap-1.5">
                            <x-icon name="yatak" class="w-3.5 h-3.5 text-slate-400" />
                            {{ $ilan->oda_sayisi }}
                        </span>
                        @endif
                    </div>
            ...
            <div class="col-span-4 py-20 text-center">
                <x-icon name="arama" class="w-12 h-12 text-slate-300 mx-auto mb-4" />
...
<div id="aiDock">
    ...
    <div class="rounded-t-2xl px-4 py-3 flex items-center justify-between cursor-pointer select-none"
        ...
        <div class="flex items-center gap-2 font-bold text-white text-sm">
            <x-icon name="robot" class="w-4 h-4" style="color:var(--gold);" />
            Yalıhan AI Asistan
        </div>
        <div class="flex items-center gap-2">
            <svg id="dockToggleIcon" class="w-4 h-4 text-white/60 hover:text-white transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M20 12H4"/></svg>
            <button onclick="event.stopPropagation(); closeDock()" class="text-white/60 hover:text-white transition-colors" aria-label="Kapat">
                <x-icon name="kapat" class="w-4 h-4" />
            </button>
        </div>
    </div>

    {{-- Dock body — glassmorphism --}}
    <div id="dockBody" class="rounded-b-2xl overflow-hidden"
         style="background:rgba(248,246,241,0.97);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border:1px solid rgba(10,22,40,0.1);border-top:none;">
        <div id="dockMessages" class="p-4 h-40 overflow-y-auto text-sm text-slate-600 space-y-2 custom-scrollbar">
            <p class="font-semibold text-xs mb-2 flex items-center gap-1.5" style="color:var(--navy);">
                <x-icon name="ampul" class="w-3.5 h-3.5" style="color:var(--gold);" />
                Örnek sorgular:
            </p>
            @foreach([
            ...
            <p class="text-xs text-slate-500 cursor-pointer hover:text-slate-800 transition-colors flex items-start gap-1.5 py-0.5"
               onclick="setAIQuery('{{ addslashes($örnek) }}')">
                <x-icon name="sag-ok" class="w-3 h-3 mt-0.5 flex-shrink-0" style="color:var(--gold);" />
                {{ $örnek }}
            </p>
            @endforeach
        </div>
        <div class="p-3" style="border-top:1px solid rgba(10,22,40,0.08);">
            <div class="flex gap-2">
                <input id="aiDockInput" type="text" placeholder="Aramak istediğinizi yazın..."
                       ...
                       class="flex-1 text-sm px-3 py-2.5 rounded-xl border outline-none focus:ring-2 focus:ring-yellow-400/60 bg-white transition-all"
                       style="border-color:rgba(10,22,40,0.15);color:var(--navy);">
                <button onclick="sendAIQuery()"
                        class="px-3.5 py-2.5 rounded-xl font-semibold text-xs transition-all hover:opacity-90 flex items-center justify-center"
                        style="background:var(--navy);color:var(--gold);">
                    <x-icon name="gonder" class="w-4 h-4" />
                </button>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    const highlightCard = document.getElementById('highlighted-card');
    if (highlightCard) {
        setTimeout(() => {
...
function toggleDock() {
    ...
    // SVG içindeki path'i değiştir: minimized iken + (expand), açık iken - (minimize)
    const isMin = dock.classList.contains('minimized');
    dockIcon.innerHTML = isMin
