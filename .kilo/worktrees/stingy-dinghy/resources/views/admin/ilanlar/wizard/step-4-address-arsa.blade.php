{{-- STEP 4: PREVIEW & APPROVAL (Mühür) --}}
<div class="space-y-6" x-data="yazlikPreviewForm()">
    <div class="mb-6">
...
function yazlikPreviewForm() {
    return {
        ...
        async approveIlan() {
            ...
            try {
                const response = await fetch(`/admin/ilanlar/${ilanId}/structured-data/approve`, {
                    method: 'POST',
                    headers: {
        ...
        async generateAIContent(type) {
            ...
            try {
                const response = await fetch(`/admin/ilanlar/${ilanId}/ai/${type}`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
