<?php

namespace App\DTOs\Ydl\Reservation;

use App\DTOs\Ydl\Reservation\Events\ReservationEvent;

/**
 * YdlReservationEvidence — Immutable evidence record for reservation operations.
 *
 * PILOT-002 Wave 1
 *
 * Every CREATE / BLOCKED / CONFLICT / OVERRIDE result is recorded as evidence.
 * Appended to ReservationEventLog after canonical execution.
 *
 * @readonly
 */
final class YdlReservationEvidence
{
    public function __construct(
        public readonly string                $eventId,
        public readonly string                $pilot,
        public readonly int                   $ilanId,
        public readonly int                   $tenantId,
        public readonly int                   $reservationId,
        public readonly string               $startDate,
        public readonly string               $endDate,
        public readonly string               $outcome,
        /** 'SUCCESS' | 'BLOCKED' | 'CONFLICT' | 'IDEMPOTENT' | 'OVERRIDE' */
        public readonly string               $ydlAuthority,
        public readonly string               $authorityContext,
        public readonly string               $canonicalResult,
        public readonly string               $executionOwner,
        public readonly string               $humanDecision,
        public readonly bool                 $success,
        public readonly ?string              $failureReason,
        public readonly ?int                $approvedBy,
        public readonly ?string              $idempotentReason,
        public readonly string               $occurredAt,
    ) {}

    public static function success(
        int    $ilanId,
        int    $tenantId,
        int    $reservationId,
        string $eventId,
        string $startDate,
        string $endDate,
        string $ydlAuthority,
        string $authorityContext,
        int    $approvedBy,
        array  $recommendation,
    ): self {
        return new self(
            eventId:           $eventId,
            pilot:             'PILOT-002',
            ilanId:            $ilanId,
            tenantId:          $tenantId,
            reservationId:     $reservationId,
            startDate:         $startDate,
            endDate:           $endDate,
            outcome:            ReservationEvent::OUTCOME_SUCCESS,
            ydlAuthority:     $ydlAuthority,
            authorityContext:   $authorityContext,
            canonicalResult:    'PropertyReservation created via ReservationService::createReservation()',
            executionOwner:     'YdlReservationOrchestrator::executeReservation()',
            humanDecision:     "Token approved by user #{$approvedBy}",
            success:           true,
            failureReason:      null,
            approvedBy:        $approvedBy,
            idempotentReason:  null,
            occurredAt:        now()->toIso8601String(),
        );
    }

    public static function blocked(
        int    $ilanId,
        int    $tenantId,
        string $eventId,
        string $startDate,
        string $endDate,
        string $ydlAuthority,
        string $authorityContext,
        string $reason,
    ): self {
        return new self(
            eventId:           $eventId,
            pilot:             'PILOT-002',
            ilanId:            $ilanId,
            tenantId:          $tenantId,
            reservationId:     0,
            startDate:         $startDate,
            endDate:           $endDate,
            outcome:            ReservationEvent::OUTCOME_BLOCKED,
            ydlAuthority:     $ydlAuthority,
            authorityContext:   $authorityContext,
            canonicalResult:    'BLOCKED — ' . $reason,
            executionOwner:     'YdlReservationOrchestrator',
            humanDecision:     'BLOCKED before execution',
            success:           false,
            failureReason:      $reason,
            approvedBy:        null,
            idempotentReason:  null,
            occurredAt:        now()->toIso8601String(),
        );
    }

    public static function conflict(
        int    $ilanId,
        int    $tenantId,
        string $eventId,
        string $startDate,
        string $endDate,
        string $ydlAuthority,
        string $authorityContext,
        int    $conflictReservationId,
    ): self {
        return new self(
            eventId:           $eventId,
            pilot:             'PILOT-002',
            ilanId:            $ilanId,
            tenantId:          $tenantId,
            reservationId:     0,
            startDate:         $startDate,
            endDate:           $endDate,
            outcome:            ReservationEvent::OUTCOME_CONFLICT,
            ydlAuthority:     $ydlAuthority,
            authorityContext:   $authorityContext,
            canonicalResult:    "Canonical lockForUpdate detected conflict with reservation #{$conflictReservationId}",
            executionOwner:     'ReservationService::createReservation()',
            humanDecision:     'BLOCKED by canonical conflict check',
            success:           false,
            failureReason:      "Conflict with existing reservation #{$conflictReservationId}",
            approvedBy:        null,
            idempotentReason:  null,
            occurredAt:        now()->toIso8601String(),
        );
    }

    public static function idempotentNoOp(
        int    $ilanId,
        int    $tenantId,
        string $eventId,
        string $startDate,
        string $endDate,
        string $ydlAuthority,
        string $authorityContext,
        string $reason,
    ): self {
        return new self(
            eventId:           $eventId,
            pilot:             'PILOT-002',
            ilanId:            $ilanId,
            tenantId:          $tenantId,
            reservationId:     0,
            startDate:         $startDate,
            endDate:           $endDate,
            outcome:            ReservationEvent::OUTCOME_IDEMPOTENT,
            ydlAuthority:     $ydlAuthority,
            authorityContext:   $authorityContext,
            canonicalResult:    'IDEMPOTENT_NO_OP',
            executionOwner:     'YdlReservationOrchestrator::executeReservation()',
            humanDecision:     'No-op (duplicate event)',
            success:           true,
            failureReason:      null,
            approvedBy:        null,
            idempotentReason:  $reason,
            occurredAt:        now()->toIso8601String(),
        );
    }

    /**
     * Convert to ReservationEvent for appending to the event log.
     */
    public function toReservationEvent(): ReservationEvent
    {
        return new ReservationEvent(
            eventId:            $this->eventId,
            pilot:              $this->pilot,
            type:               match ($this->outcome) {
                ReservationEvent::OUTCOME_SUCCESS     => ReservationEvent::TYPE_CREATED,
                ReservationEvent::OUTCOME_BLOCKED   => ReservationEvent::TYPE_BLOCKED,
                ReservationEvent::OUTCOME_CONFLICT   => ReservationEvent::TYPE_CONFLICT,
                ReservationEvent::OUTCOME_IDEMPOTENT => ReservationEvent::TYPE_IDEMPOTENT,
                default                              => ReservationEvent::TYPE_CREATED,
            },
            outcome:            $this->outcome,
            ilanId:            $this->ilanId,
            tenantId:          $this->tenantId,
            reservationId:     $this->reservationId,
            startDate:        $this->startDate,
            endDate:          $this->endDate,
            ydlAuthority:     $this->ydlAuthority,
            authorityContext:   $this->authorityContext,
            canonicalResult:    $this->canonicalResult,
            executionOwner:    $this->executionOwner,
            humanDecision:     $this->humanDecision,
            userId:            $this->approvedBy,
            occurredAt:        $this->occurredAt,
        );
    }

    public function toArray(): array
    {
        return [
            'event_id'           => $this->eventId,
            'pilot'             => $this->pilot,
            'ilan_id'           => $this->ilanId,
            'tenant_id'         => $this->tenantId,
            'reservation_id'    => $this->reservationId,
            'start_date'        => $this->startDate,
            'end_date'          => $this->endDate,
            'outcome'           => $this->outcome,
            'ydl_authority'     => $this->ydlAuthority,
            'authority_context'  => $this->authorityContext,
            'canonical_result'  => $this->canonicalResult,
            'execution_owner'   => $this->executionOwner,
            'human_decision'   => $this->humanDecision,
            'success'           => $this->success,
            'failure_reason'    => $this->failureReason,
            'approved_by'       => $this->approvedBy,
            'idempotent_reason' => $this->idempotentReason,
            'occurred_at'      => $this->occurredAt,
        ];
    }
}
