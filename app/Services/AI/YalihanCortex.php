...
use App\Services\CRM\KisiAnalyticsService;
use App\Services\CRMIntelligenceService;
use App\Services\Integrations\N8nIntegrationService;
use App\Services\Integrations\TelegramAIBotService;
...
class YalihanCortex
{
     ...
     */
    protected TKGMLearningService $tkgmLearning;
     ...
     */
    protected TelegramAIBotService $telegramBot;
     ...
     */
    protected MarketAnalysisService $marketService;
    ...
    public function __construct(
        ...
        TKGMLearningService $tkgmLearning,
        PredictiveAnalyticsEngineService $predictiveEngine,
        ...
        TelegramAIBotService $telegramBot,
    ...
    public function generateIlanDescription($ilan, array $options = []): array
    {
        ...
        $data['length'] = $options['length'] ?? 'medium';
        ...
        try {
            ...
            if (empty($description)) {
                return [
                    'success' => false,
                    'error' => 'Empty description returned from AI service',
                    'provider' => 'ollama',
                ];
            }
    ...
    protected function getProviderCandidates(string $taskType): array
    {
        try {
            ...
            LogService::warning('YalihanCortex::getProviderCandidates failed', [
                'error' => $e->getMessage(),
            ]);
    ...
    {
        $response = app(AnalyzePropertyAction::class)->handle([
    ...
    {
        $response = app(ExtractPropertyFeaturesAction::class)->handle([
            'text' => $text,
        ]);
    ...
    {
        $response = app(SuggestPropertyTemplateAction::class)->handle([
            'category_name' => $categoryName,
            'description' => $description,
        ]);
    ...
    {
        $response = app(GeneratePropertyTemplateAction::class)->handle(array_merge($options, [
            'kategori' => $kategori,
    ...
    {
        return app(IntelligenceHub::class)->getListingHealth($ilanId);
    }
    ...
    {
        return app(BriefingService::class)->generateDailyBriefing();
    }
    ...
    {
        return app(AIOrchestrator::class)->simpleAnalysis($data, $context);
    }
    ...
    {
        return app(AIOrchestrator::class)->getPriceSuggestionsMetrics($data);
    }

    /**
     * Prepares a temporary Talep DTO for matching.
     */
    public function prepareTemporaryTalep(array $data): Talep
    {
        return app(AIOrchestrator::class)->createMatchTalep($data);
    }
    ...
    {
        return app(AIOrchestrator::class)->queueVideoRender($ilanId);
    }
    ...
    {
        return app(AIOrchestrator::class)->generateDescriptionFallback($baslik, $tip, $kategori, $il, $ilce);
    }
     ...
     */
    public function requestCustomerRecommendations(Kisi|\App\Models\Lead $target, int $limit = 5): array
    {
        return app(CRMIntelligenceService::class)->getRecommendedListings($target, $limit);
    }
    ...
    {
        return app(KisiAnalyticsService::class)->getAIAnalysis($id);
    }
