...
class CortexContentService
{
    ...
    public function generateMultilingualTitle($ilan, string $language = 'en', array $options = []): array
    {
        ...
        try {
            ...
            $durationMs = LogService::stopTimer($startTime);
            Log::error('Multilingual title generation failed: ' . $e->getMessage(), ['exception' => $e]);
    ...
    public function generateMultilingualDescription($ilan, string $language = 'en', array $options = []): array
    {
        ...
        try {
            ...
            $result = [
                'success' => $multilingualResult['success'] ?? false,
                'description' => $multilingualResult['description'] ?? '', // context7-ignore
                'language' => $language,
                'ilan_id' => $ilanModel->id,
                'metadata' => [
                    'processed_at' => now()->toISOString(),
            ...
            $durationMs = LogService::stopTimer($startTime);
            Log::error('Multilingual description generation failed: ' . $e->getMessage(), ['exception' => $e]);
    ...
    public function optimizeIlanTitle(array $ilanData): array
    {
        ...
        try {
            ...
            $this->telemetry->logTransaction(
                ...
                [
                    'request' => $ilanData,
                    'response' => ['title' => $optimizedTitle] // context7-ignore
                ]
            );
    ...
    /**
     * Get content readiness summary and SEO score.
     */
    public function getContentSummary(Ilan $ilan): array
    {
        $hasTitle = ! empty(trim($ilan->baslik ?? ''));
        $hasDescription = ! empty(trim($ilan->aciklama ?? ''));

        $seoScore = 0;
        if ($hasTitle) {
            $titleLen = mb_strlen($ilan->baslik);
            $seoScore += ($titleLen >= 40 && $titleLen <= 70) ? 50 : 25;
            if ($ilan->il?->il_adi && strpos($ilan->baslik, $ilan->il->il_adi) !== false) {
                $seoScore += 25;
            }
            if ($ilan->anaKategori?->name && strpos($ilan->baslik, $ilan->anaKategori->name) !== false) {
                $seoScore += 25;
            }
        }
        ...
        return [
            ...
            'title_seo_score' => min(100, $seoScore),
            'ready_for_publish' => $hasTitle && $hasDescription,
        ];
    }
}
