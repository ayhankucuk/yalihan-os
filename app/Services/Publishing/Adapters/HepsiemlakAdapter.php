...
use App\DTOs\Publishing\HepsiemlakFormatDTO;
...
/**
 * Hepsi Emlak Adapter — Sprint 6.5
 *
 * Hepsiemlak.com formatına dönüştürür.
 *
 ...
 */
class HepsiemlakAdapter implements ChannelAdapterContract
{
    private const CHANNEL = 'hepsiemlak';
    ...
    /**
     * Bu ilan Hepsiemlak için uygun mu?
     * Tüm emlak tiplerini destekler.
     */
    public function supports(Ilan $ilan): bool
    {
        // Hepsiemlak: Türkiye geneli tüm emlak
        // Bodrum/Muğla bölgesine odaklan
    ...
    /**
     * Zorunlu alanlar: Hepsiemlak için minimum gereksinimler.
     *
     * @return string[]
     */
    ...
    /**
     * Ilan + Vision verilerini Hepsiemlak formatına dönüştürür.
     *
    ...
    public function buildPayload(
        ...
        $baslik = $this->titleTransformer->forHepsiemlak($ilan, $titleHints);

        // 4. Description
        $descriptionData = $this->descriptionTransformer->forHepsiemlak($ilan, $media);

        // 5. Room/space mapping
        $roomData = $this->roomTypeMapper->forHepsiemlak($ilan, $media);
        ...
        $features = $this->amenityMapper->toHepsiemlakFeatures($detectedAmenities, $detectedLuxury);

        // 7. Photos
        $photos = $this->buildPhotoPayload($ilan, $media);

        // 8. HepsiemlakFormatDTO oluştur
        $hepsiemlakDto = new HepsiemlakFormatDTO(
            ilanId: $ilan->id,
        ...
        return new ChannelPayloadDTO(
            ...
            ilanId: $ilan->id,
            mappedFields: $hepsiemlakDto->toArray(),
            photos: $photos,
            seo: [
    ...
    /** @return array<array{baslik: string|null, url: string, birincil: bool, sira: int}> */
