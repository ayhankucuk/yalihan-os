...
class SabScanRunner
{
     ...
     */
    public function scan(string $path = 'app', ?array $specificFiles = null): array
    {
        $violations = [];
        ...
        if ($specificFiles !== null) {
            foreach ($specificFiles as $absolutePath) {
                $rootRelativePath = str_replace(base_path() . '/', '', $absolutePath);
    ...
    private function normalizeViolation(array $issue, string $origin): array
    {
        ...
        $type = $issue['type'] ?? ''; // Preserve original type for baseline matching // context7-ignore
