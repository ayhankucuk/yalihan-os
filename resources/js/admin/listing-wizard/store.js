    ...
    async fetchConfig(yayinTipiId) {
      ...
      try {
        ...
        if (data.success) {
          ...
          console.log(
            "[Alpine Store] Config loaded successfully:",
            data.config,
          );
        } else {
        ...
        // ✅ Downgraded to warn: /admin/api/type-config endpoint may not be
        // provisioned for all yayin_tipi IDs. Fallback config is applied below,
        // so this is graceful degradation — not a fatal error.
        console.warn(
          "[Alpine Store] fetchConfig degraded (fallback applied):",
          error.message,
        );
