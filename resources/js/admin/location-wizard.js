  ...
  return {
    ...
    initMap() {
      ...
      if (typeof L === "undefined") {
        console.warn("⚠️ Leaflet JS bulunamadı (graceful)");
        const fallback =
    ...
    async fetchDistricts() {
      ...
      try {
        const response = await fetch(
          `/api/v1/location/districts/${this.selectedCity}`,
        );
        ...
        console.warn("İlçeler yüklenemedi (graceful):", error);
      } finally {
    ...
    async fetchNeighborhoods() {
      ...
      try {
        ...
        console.warn("Mahalleler yüklenemedi (graceful):", error);
      } finally {
    ...
    _isValidLatLng(lat, lng) {
      ...
      return (
        l !== null && g !== null && l >= -90 && l <= 90 && g >= -180 && g <= 180
      );
    },
    ...
    focusNeighborhood() {
      ...
      const hood = this.neighborhoods.find(
        (h) => h.id == this.selectedNeighborhood,
      );
      ...
      if (hood && (hood.lat || hood.latitude) && (hood.lng || hood.longitude)) {
        ...
        // Debounce prevent?
        fetch(
          `/api/v1/location/neighborhood/${this.selectedNeighborhood}/coordinates`,
        )
          .then((r) => r.json())
          .then((result) => {
            if (
          ...
          .catch((e) =>
            console.warn("Mahalle koordinatı alınamadı (graceful)", e),
          );
      }
    },
    ...
    flyToLocation(lat, lng, zoom = 13) {
      ...
      if (!this._isValidLatLng(lat, lng)) {
        console.warn(
          `⚠️ Invalid flyTo coordinates: ${lat}, ${lng}. Defaulting to Bodrum.`,
        );
        // Fallback to Bodrum
        lat = 37.0344;
        lng = 27.4305;
