...
document.addEventListener("alpine:init", () => {
  Alpine.data("schemaFieldRenderer", () => ({
    ...
    init() {
      ...
      this.kategoriId =
        this.$el?.dataset?.kategoriId ||
        document.getElementById("alt_kategori_id")?.value ||
        null;
      this.yayinTipiId =
        this.$el?.dataset?.yayinTipiId ||
      ...
      window.addEventListener("beforeunload", (e) => {
        if (this.isDirty || this.saveStatus === "saving") {
          e.preventDefault();
          e.returnValue =
            "Kaydedilmemiş değişiklikleriniz var. Ayrılmak istediğinize emin misiniz?";
        }
      });
    ...
    async persistField(field, value, attempt = 0) {
      ...
      try {
        ...
        console.warn("Auto-save error (graceful):", err);
        ...
        if (attempt < this.maxRetries && !this.fieldErrors[field]) {
          ...
          console.log(
            `🔄 Retrying save in ${delay}ms... (Attempt ${attempt + 1})`,
          );
    ...
    async loadSchema() {
      ...
      try {
        ...
        console.log(
          `✅ Schema loaded: ${this.fields.length} fields for ${this.meta.kategori_slug}/${this.meta.yayin_tipi_slug}`,
        );

        // Dispatch schema-loaded event for other components
        window.dispatchEvent(
          new CustomEvent("wizard:schema-loaded", {
            detail: {
              fields: this.fields,
              meta: this.meta,
        ...
        console.warn("Schema yükleme hatası (graceful):", err);
        this.error = err.message;
        this.fields = [];
        this.groupedFields = [];
      } finally {
    ...
    async fetchAiSuggestions() {
      ...
      try {
        const response = await fetch(
          `/api/v1/wizard/ai-suggestions/${this.draftId}`,
        );
        const result = await response.json();
          ...
          if (count > 0) {
            ...
            window.dispatchEvent(
              new CustomEvent("notification", {
                detail: {
                  type: "success",
                  message: `${count} alanda akıllı öneri bulundu! 🪄`,
                },
              }),
            );
          }
        ...
        console.warn("AI Suggestions error (graceful):", err);
      } finally {
    ...
    get completionPercentage() {
      ...
      const requiredFields = this.fields.filter(
        (f) => f.required && this.isFieldVisible(f),
      );
    ...
    dispatchAutoSave() {
      window.dispatchEvent(
        new CustomEvent("wizard:auto-save", {
          detail: {
            step: 2,
