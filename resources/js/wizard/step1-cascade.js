...
export function loadAltKategoriler(anaKategoriId) {
  ...
  console.log(
    "[Cascade] loadAltKategoriler called, anaKategoriId:",
    anaKategoriId,
    "url:",
    subcategoriesUrl,
  );

  logger.log("🔍 Alt kategori yükleme başlatıldı:", {
    anaKategoriId,
  ...
  return fetch(subcategoriesUrl)
    ...
    .catch((error) => {
      logger.warn("Alt kategoriler yüklenemedi (graceful):", error);
...
export function loadYayinTipleri(altKategoriId) {
  ...
  return fetch(publicationTypesUrl)
    ...
    .catch((error) => {
      logger.warn("Yayın tipleri yüklenemedi (graceful):", error);
...
export async function quickSelectCategory(config) {
  ...
  if (anaId) {
    anaOption = Array.from(anaSelect.options).find(
      (o) => String(o.value) === String(anaId),
    );
  }
  ...
  if (!anaOption) {
    logger.warn("Ana kategori bulunamadı (graceful):", anaSlug, anaId);
    return;
  }
  ...
  if (altId) {
    altOption = Array.from(altSelect.options).find(
      (o) => String(o.value) === String(altId),
    );
  }
  ...
  if (tipId) {
    tipOption = Array.from(tipSelect.options).find(
      (o) => String(o.value) === String(tipId),
    );
  }
  ...
  if (!tipOption) {
    ...
    logger.log(
      "🕵️ Quick Select Available Types:",
      availableOptions,
      "Searching for:",
      tipSlug,
    );
  ...
  if (tipOption) {
    ...
    logger.warn("Yayın tipi bulunamadı (graceful):", tipSlug);
  }
}
...
export function dispatchCategoryChangedEvent() {
  ...
  const yayinTipiName =
    yayinTipiSelect.options[yayinTipiSelect.selectedIndex]?.text;
  ...
  fetch(districtsUrl)
    ...
    .then((data) => {
      ...
      if (Array.isArray(districts) && districts.length > 0) {
        ...
        // P1-FIX: Muğla (id=48) seçildiğinde Bodrum'u otomatik seç
        // Bodrum ilçe ID'si veritabanından gelir; genellikle 898 veya benzeri bir ID'dir.
        // Kullanıcı il seçtiğinde ilçe otomatik gelir ve harita da Bodrum'a odaklanır.
        if (parseInt(ilId) === 48 && districts.length > 0) {
          // Bodrum'un ID'sini bul (veritabanında genellikle ilk sırada veya adı "Bodrum" olan)
          const bodrumOption = Array.from(ilceSelect.options).find(
            opt => (opt.text || '').toLowerCase().includes('bodrum')
          );
          if (bodrumOption && bodrumOption.value) {
            ilceSelect.value = bodrumOption.value;
            ilceSelect.dispatchEvent(new Event('change', { bubbles: true }));
            console.info('[WIZARD] P1-FIX: Muğla auto-selected Bodrum ilçe:', bodrumOption.value);
          }
        }
      } else {
    ...
    .catch((error) => {
      logger.warn("İlçeler yüklenemedi (graceful):", error);
...
export function loadMahalleler(ilceId) {
  ...
  fetch(neighborhoodsUrl)
    ...
    .catch((error) => {
      logger.warn("Mahalleler yüklenemedi (graceful):", error);
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => {
      ...
      if (!junctionEl) {
        ...
        console.log(
          "[SSOT Guard] ✅ #junction_id element mevcut — Cascade hazır.",
        );
      }
    });
  }
