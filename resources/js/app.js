// ✅ Observability Layer - Global Telemetry (Production Monitoring)
import './global.js';
...
import vacationPricingManager from './wizard/components/vacation-pricing.js';

Alpine.plugin(collapse);

window.Alpine = Alpine;
window.DynamicFormHandler = DynamicFormHandler;
window.vacationPricingManager = vacationPricingManager;

// Alpine'i başlatmadan önce tüm modüllerin yüklenmesini bekle
// ✅ SAB: Select2 Bridge Directive
document.addEventListener('alpine:init', () => {
    Alpine.directive('select2', (el, { expression }, { evaluate }) => {
        $(el)
            .select2()
            .on('select2:select change', (e) => {
...
// ✅ SAB: Alpine.js başlatma - DOM element kontrolü ile güvenli başlatma
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        // Modüller yüklenmesi için kısa bir bekleme
        setTimeout(() => {
            try {
                // ✅ SAB: Alpine başlatmadan önce DOM'un hazır olduğundan emin ol
...
class MobileFirstResponsive {
    ...
    enableOfflineFeatures() {
        ...
        document.querySelectorAll('[data-offline]').forEach((element) => {
            element.classList.add('internet_durumu_sinyal');
        });
    }
    ...
    optimizeForMobile() {
        ...
        if (window.touchGestures) {
            document.querySelectorAll('[data-kaydirma-durumu]').forEach((element) => {
    ...
    enableHoverEffects() {
        ...
        document.querySelectorAll('[data-hover-effect]').forEach((element) => {
            element.classList.add('etkilesim_durumu_sinyal');
        });
    }
