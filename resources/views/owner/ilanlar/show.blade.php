...
@section('content')
    <div class="mb-6 flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex items-center gap-3">
            @if ($ilan->yayindami)
                <span
                    class="inline-flex items-center gap-1.5 rounded-md bg-green-50 px-3 py-1.5 text-sm font-medium text-green-700 ring-1 ring-inset ring-green-600/20 dark:bg-green-900/30 dark:text-green-400 dark:ring-green-500/20">
                    <span class="h-1.5 w-1.5 rounded-full bg-green-500"></span>
                    Yayında
                </span>
            @else
                <span
                    class="inline-flex items-center gap-1.5 rounded-md bg-gray-50 px-3 py-1.5 text-sm font-medium text-gray-600 ring-1 ring-inset ring-gray-500/10 dark:bg-gray-800 dark:text-gray-400 dark:ring-gray-700">
    ...
    @php
        $bc001Components = [
            ...
            [
                ...
                'detail' =>
                    $ilan->workspace?->metadata_json['notebook_synced'] ?? false
                        ? 'Memory Güncellendi'
                        : 'Bilgi Kaydediliyor...',
            ],
            [
            ...
            [
                ...
                'detail' =>
                    $ilan->workspace?->metadata_json['publish_queue']['enqueued_at'] ?? false
                        ? 'CRM Kaydedildi'
                        : 'CRM Oluşturuluyor...',
            ],
            [
                ...
                'detail' =>
                    $ilan->workspace?->lifecycle_state?->value === 'READY_FOR_PUBLISH'
                        ? 'Yayınlamaya Hazır'
                        : 'Yayınlama Hazırlanıyor...',
            ],
            [
                'key' => 'telegram',
                'label' => 'Bildirim',
        ...
        $bc001Score =
            $ilan->workspace?->ai_completion_percent ??
            ($ilan->fotograflar && $ilan->fotograflar->count() > 0 ? 25 : 0);
    @endphp

    <div class="mb-6 rounded-xl border bg-gradient-to-br from-[#0A1628] to-[#0f2040] p-6 shadow-lg" x-data="{
        ...
        <div class="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4" x-ref="readinessPanel">
            @foreach ($bc001Components as $component)
                ...
                @if ($component['ready']) border-[#C9A84C]/40 bg-[#C9A84C]/10
                @else
                    border-[#F8F6F1]/10 bg-[#F8F6F1]/5 @endif
            ">
        ...
        <div class="rounded-lg border border-[#F8F6F1]/10 bg-[#F8F6F1]/5 p-4">
            ...
            @if ($bc001AllReady)
                <div class="mt-4 flex items-center gap-3">
                    ...
                    <form id="bc001-approve-form" method="POST" action="{{ route('owner.ilanlar.yayinla', $ilan->id) }}">
                        @csrf
                    </form>
                </div>
            @else
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800"
                x-data="ownerPhotoUpload({{ json_encode([
                    'ilanId' => $ilan->id,
                ...
                @if ($ilan->fotograflar && $ilan->fotograflar->count() > 0)
                    ...
                    <div class="relative h-64 sm:h-96 w-full">
                        <img src="{{ Storage::url($kapak->dosya_yolu) }}" alt="{{ $ilan->baslik }}"
                            class="h-full w-full object-cover">
                    </div>
                @else
                    <div
                        class="flex h-64 w-full items-center justify-center bg-gray-50 text-gray-400 dark:bg-slate-800 dark:text-slate-500">
                ...
                <div class="border-t border-gray-100 p-4 dark:border-slate-700">
                    ...
                    <div x-show="message" x-transition
                        class="mb-3 rounded-lg bg-green-50 p-3 text-sm text-green-700 dark:bg-green-900/20 dark:text-green-400"
                        x-text="message"></div>
                    <div x-show="error" x-transition
                        class="mb-3 rounded-lg bg-red-50 p-3 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-400"
                        x-text="error"></div>
                        ...
                        <template x-for="(file, index) in selectedFiles" :key="index">
                            <div class="relative">
                                <img :src="file.preview"
                                    class="h-16 w-16 rounded-lg object-cover ring-1 ring-gray-200 dark:ring-slate-600">
                                <button type="button" @click="removeFile(index)"
                                    class="absolute -top-1 -right-1 flex h-5 w-5 items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-600">
                    ...
                    <div class="flex items-center gap-3">
                        <label
                            class="cursor-pointer rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 shadow-sm hover:bg-gray-50 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-200 dark:hover:bg-slate-600">
                            <span>Fotoğraf Seç</span>
                            <input type="file" name="photos[]" multiple accept="image/jpeg,image/png,image/webp"
                                class="hidden" @change="handleFileSelect($event)" :disabled="uploading">
                        </label>
                        ...
                        <span class="text-xs text-gray-500 dark:text-slate-400">JPG, PNG, WEBP — Max 5MB × 10
                            fotoğraf</span>
                    </div>
                </div>
                        ...
                        <div class="grid grid-cols-4 gap-2">
                            @foreach ($ilan->fotograflar as $foto)
                                <div class="group relative">
                                    <img src="{{ Storage::url($foto->dosya_yolu) }}"
                                        class="h-16 w-full rounded-lg object-cover ring-1 ring-gray-200 dark:ring-slate-600">
                                    <button type="button"
                                        ...
                                        <svg class="h-5 w-5 text-white" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 7l-.867 12.142A2 2 0 0110.138 21H5.79a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-9V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v8M5 7h14a1 1 0 011 1v12a1 1 0 01-1 1H5a1 1 0 01-1-1V7a1 1 0 011-1z" />
                                        </svg>
                                    </button>
                                </div>
                ...
                <div class="px-5 py-5">
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                {{ $ilan->anaKategori->isim ?? '-' }} @if ($ilan->altKategori)
                                    / {{ $ilan->altKategori->isim }}
                                @endif
                            </dd>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-white">
                                {{ $ilan->oda_sayisi ?? '-' }} @if ($ilan->salon_sayisi)
                                    + {{ $ilan->salon_sayisi }}
                                @endif
                            </dd>
                        </div>
                    ...
                    @if ($ilan->aciklama)
                        <div class="mt-6 border-t border-gray-100 pt-6 dark:border-slate-700">
                            ...
                            <dd
                                class="mt-2 text-sm text-gray-900 dark:text-slate-300 prose prose-sm max-w-none dark:prose-invert">
        ...
        <div class="space-y-6">
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800"
                x-data="ownerReadiness()" x-init="loadReadiness()">
                {{-- Başlık --}}
                ...
                <div x-show="!loading && data" class="mb-3">
                    ...
                    <div class="h-2 w-full overflow-hidden rounded-full bg-gray-200 dark:bg-slate-700">
                        <div class="h-full rounded-full transition-all duration-500" :class="barColor"
                        ...
                        <template x-for="field in data.missing_fields" :key="field.field">
                            <li class="flex items-start gap-1.5 text-xs text-gray-600 dark:text-slate-300">
                                <svg class="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-amber-500" fill="none"
                                    ...
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 9v2m0 2h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                </svg>
                ...
                <div x-show="!loading && data && data.recommendations && data.recommendations.length > 0"
                    class="mt-4 border-t border-gray-100 pt-4 dark:border-slate-700">
                    ...
                    <ul class="space-y-2">
                        <template x-for="rec in data.recommendations" :key="rec.field">
                            <li
                                class="rounded-lg border border-indigo-100 bg-indigo-50/50 p-2 dark:border-indigo-900/30 dark:bg-indigo-900/10">
                                <div class="flex items-start gap-2">
                                    <span
                                        class="mt-0.5 flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-full text-[10px] font-bold"
                                        :class="{
                                            'bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-300': rec
                                                .priority === 'critical',
                                            'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300': rec
                                                .priority === 'high',
                                            'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300': rec
                                                .priority === 'medium',
                                            'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400': rec
                                                .priority === 'low'
                                        }"
                                        x-text="rec.priority.charAt(0).toUpperCase()"></span>
                                    <div class="flex-1">
                                        <p class="text-xs font-medium text-gray-700 dark:text-slate-200"
                                            x-text="rec.recommendation"></p>
                                        <p class="mt-0.5 text-[10px] text-indigo-600 dark:text-indigo-400"
                                            x-text="'→ ' + rec.action_label"></p>
                                    </div>
                                </div>
                            </li>
                        </template>
                ...
                <div x-show="!loading && data && data.next_best_action"
                    class="mt-4 rounded-lg border border-indigo-200 bg-indigo-50 p-3 dark:border-indigo-800 dark:bg-indigo-900/20">
                    <div class="flex items-start gap-2">
                        <svg class="mt-0.5 h-4 w-4 flex-shrink-0 text-indigo-600 dark:text-indigo-400" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <div>
                            ...
                            <p class="mt-0.5 text-xs text-indigo-600 dark:text-indigo-400" x-text="data.next_best_action">
                            </p>
                        </div>
                    </div>
                </div>
                {{-- Tamamlandı mesajı --}}
                <div x-show="!loading && data && (!data.missing_fields || data.missing_fields.length === 0)"
                    class="mt-3">
            ...
            {{-- ADR-001 Phase 1B: AI Piyasa Değerleme Widget --}}
            @if ($valuation && ($valuation['is_success'] ?? false))
                @php $vd = $valuation['data']; @endphp
                <div
                    class="rounded-xl border border-indigo-200 bg-white p-5 shadow-sm dark:border-indigo-800 dark:bg-slate-800">
                    <h3 class="mb-4 text-sm font-semibold text-indigo-600 dark:text-indigo-400">AI Piyasa Analizi</h3>

                    {{-- TAHMİNİ PİYASA DEĞERİ --}}
                    <div class="mb-4">
                        <div class="text-xs text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1">TAHMİNİ PİYASA DEĞERİ</div>
                        <div class="text-2xl font-bold text-gray-900 dark:text-white">
                            {{ number_format($vd['estimated_value'], 0, ',', '.') }}
                            <span class="text-sm font-normal text-gray-500">TRY</span>
                        </div>
                        <div class="mt-1 text-xs text-gray-500 dark:text-slate-400">
                            {{ number_format($vd['price_range_low'], 0, ',', '.') }} —
                            {{ number_format($vd['price_range_high'], 0, ',', '.') }}
                        </div>
                    </div>

                    {{-- Fiyat Karşılaştırması --}}
                    @if (isset($ilan->fiyat) && $ilan->fiyat > 0)
                        @php
                            $diff = (($ilan->fiyat - $vd['estimated_value']) / max(1, $vd['estimated_value'])) * 100;
                            $diffLabel =
                                $diff > 2
                                    ? 'Piyasanın Üstünde'
                                    : ($diff < -2
                                        ...
                                        : 'Piyasayla Uyumlu');
                        @endphp
                        <div class="mb-4 rounded-md bg-gray-50 px-3 py-2 text-sm dark:bg-slate-700">
                            <span class="text-gray-600 dark:text-slate-300">Piyasanın</span>
                            <span
                                class="font-semibold {{ $diff > 2 ? 'text-amber-600' : ($diff < -2 ? 'text-green-600' : 'text-blue-600') }}">
                                {{ $diff > 2 ? 'Üstünde' : ($diff < -2 ? 'Altında' : 'Uyumlu') }}
                            </span>
                    ...
                    <div class="mb-3 flex items-center justify-between text-sm">
                        <span class="text-gray-500 dark:text-slate-400">Piyasa Trendi</span>
                        <span
                            class="font-medium {{ ($vd['market_trend'] ?? 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400' }}">
                            {{ $vd['market_trend'] >= 0 ? '+' : '' }}{{ number_format($vd['market_trend'], 1) }}%
                        </span>
                    </div>

                    {{-- Güven Skoru --}}
                    @php
                        $confidence = $vd['confidence_score'] ?? 0;
                        $confidenceLabel =
                            $confidence >= 80 ? 'Yüksek Güven' : ($confidence >= 60 ? 'Orta Güven' : 'Düşük Güven');
                        $confidenceClass =
                            $confidence >= 80
                    ...
                    @endphp
                    <div class="mb-3 flex items-center justify-between text-sm">
                        <span class="text-gray-500 dark:text-slate-400">Güven Skoru</span>
                        <span
                            class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {{ $confidenceClass }}">
                            {{ $confidenceLabel }}
                        </span>
                    </div>
                    ...
                    {{-- Likidite --}}
                    @php
                        $liquidity = $vd['liquidity_score'] ?? 'MEDIUM';
                        $liquidityLabel = $liquidity === 'HIGH' ? 'Yüksek' : ($liquidity === 'LOW' ? 'Düşük' : 'Orta');
                    @endphp
                    <div class="flex items-center justify-between text-sm">
                        <span class="text-gray-500 dark:text-slate-400">Likidite</span>
                        <span class="font-medium text-gray-900 dark:text-white">{{ $liquidityLabel }}</span>
                    </div>

                    <div class="mt-3 text-xs text-gray-400 dark:text-slate-500">
                        {{ $vd['comparable_count'] ?? 0 }} emsal kayıt analiz edildi.
                    </div>
                </div>
            @endif
            ...
            @if ($ilan->danisman && $ilan->danisman->id)
                <div
                    class="rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                    <h3 class="mb-4 text-sm font-medium text-gray-500 dark:text-slate-400">Sorumlu Danışman</h3>
                    <div class="flex items-center gap-4">
                        <div
                            class="flex h-12 w-12 items-center justify-center rounded-full bg-blue-100 text-lg font-bold text-blue-700 dark:bg-blue-900 dark:text-blue-300">
                        ...
                        <div>
                            ...
                            @if ($ilan->danisman->phone_number)
                                <div class="text-sm text-gray-500 dark:text-slate-400">{{ $ilan->danisman->phone_number }}
                                </div>
                            @endif
                        </div>
                    </div>
                    <div class="mt-4">
                        <a href="mailto:{{ $ilan->danisman->email }}"
                            class="block w-full rounded-md bg-blue-50 px-3 py-2 text-center text-sm font-medium text-blue-700 hover:bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400 dark:hover:bg-blue-900/50">
                            Mesaj Gönder
                        </a>
                    </div>
                </div>
